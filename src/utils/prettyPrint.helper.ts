...
type chalkAnimationTypes = 'rainbow' | 'pulse' | 'glitch' | 'radar' | 'neon' | 'karaoke';
...
export function printAnimatedText(text: string, animation: chalkAnimationTypes = 'neon') {
...
export function printWithBadge(options: CLIPrefixOptions): string {
    ...
    if ((chalk as any)[color]) {
        cliPrefix = `${(chalk as any)[color]('>')} ${(chalk as any).reset.inverse.bold[color](` ${badgeName} `)}`;
    } else {
        cliPrefix = `${chalk.keyword(color)('>')}${chalk.reset.inverse.bold.keyword(color)(` ${badgeName} `)}`;
    }
