...
export function changeDirectory(targetDir: string): void {
  try {
    ...
    new OrbitLogger('DIRECTORY-CHANGER').info(`Changed directory to: ${targetDir}`);
  } catch (error) {
    new OrbitLogger('DIRECTORY-CHANGER').error(`Error changing directory: ${error.message}`);
    throw error;
  }
}
 ...
 * @returns {void}
 */
export function createDirectory(options: string, paths: string | string[]): void {
  paths = typeof paths === 'string' ? [paths] : paths;
  const dirTypo = typeof paths === 'string' ? 'Directory' : 'Directories';

  try {
    shell.mkdir(options, paths);
    new OrbitLogger('DIRECTORY-CREATOR').info(`${dirTypo} created: ${paths.join(', ')}`);
  } catch (error) {
    new OrbitLogger('DIRECTORY-CREATOR').error(`Error creating ${dirTypo}: ${error.message}`);
    throw error;
  }
}
...
export function moveFile(sources: string | string[], dest: string): void {
  try {
    ...
    new OrbitLogger('FILE-MOVER').info(`Moved ${Array.isArray(sources) ? sources.join(', ') : sources} to ${dest}`);
  } catch (error) {
    new OrbitLogger('FILE-MOVER').error(`Error moving file(s): ${error.message}`);
    throw error;
  }
}
...
export function removeFile(options: string, files: string[]): void {
  try {
    ...
    new OrbitLogger('FILE-REMOVER').info(`Removed ${Array.isArray(files) ? files.join(', ') : files}`);
  } catch (error) {
    new OrbitLogger('FILE-REMOVER').error(`Error removing file(s): ${error.message}`);
    throw error;
  }
}
 ...
 */
export function replaceInFiles(dir: string, searchStr: string, replaceStr: string) {
